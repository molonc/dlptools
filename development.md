...
git checkout -b mynewfunc

# 4. code happy.
```

If you haven’t worked on an R package before,
[this](https://r-pkgs.org/whole-game.html) is a good read to get
started.

To add functions, we can rely on
[`library(devtools)`](https://devtools.r-lib.org/) to help with the
metadata stuff and package construction. Steps include:

1.  boot an R instance and set the working directory to wherever you
    cloned this repo
...
2.  add your R function either within an existing `R/*R` module, or
    create a new module
...
3.  To test your function, in the R instance you can load it and check
    it out
...
4.  Then in your R instance, we can build the needed meta data for the
    new function.
...
If your function includes code from another R package (even base R), you
need to declare that and add it to the package metadata (unless it’s
already listed in the `Imports:` section of the `DESCRIPTION` file).
...
5b. Might need to update vignettes to show off your function

Remember, vignettes use the *installed* version, so you can install it
with:
...
# and submit a PR to the dev branch.
```
