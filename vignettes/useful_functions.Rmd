...
dlptools::bin_within_centromere(
  reads_df = ex_reads,
  ...
  bin_location_column = "start" # column to use for the location of a bin,
  # default is "start". So the start of a bin in checked if it falls within the
  # extent of the centromere, plus any specified padding.
) |>
  dplyr::count(cell_id, within_centro) |>
...
filt_reads <- raw_reads |>
  ...
  dlptools::bin_within_centromere(padding = 3e6) |>
  dplyr::filter(
    gc != -1 & map > 0.99 & !within_centro & !mask
  )


tibble::tibble(
  raw_bins = dplyr::distinct(raw_reads, chr, start, end) |> dplyr::count(),
  filtered_bins = dplyr::distinct(filt_reads, chr, start, end) |> dplyr::count()
)
```
...
```{r}
# e.g., we can add some other fake cell level meta data to carry through for
# missing bins
